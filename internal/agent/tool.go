...
func (r *ToolRegistry) buildTool(td model.Tool) Tool {
	...
	case model.HandlerScript:
		var cfg model.ScriptHandlerConfig
		if err := json.Unmarshal(td.HandlerConfig, &cfg); err != nil {
			log.WithError(err).WithField("tool", td.Name).Warn("[Tool] invalid script handler config, skipping")
			return nil
		}
		return &dynamicTool{
			toolName: td.Name,
			toolDesc: td.Description,
			handler:  tools.NewScriptHandler(cfg, td.TimeoutSeconds()),
		}
	default:
...
func (t *trackedTool) Call(ctx context.Context, input string) (string, error) {
	...
	recordCtx := ctx
	if t.name == "sub_agent" {
		recordCtx = withSubAgentDepth(ctx, subAgentDepth(ctx)+1)
	}
	t.tracker.RecordStep(recordCtx, model.StepToolCall, t.name, input, output, status, errMsg, duration, 0, meta)
	return output, err
}
