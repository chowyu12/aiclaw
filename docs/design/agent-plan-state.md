...
- Harness 终态为 `completed/skipped/blocked/failed`；`pending/running` 仍需继续执行或明确收口。
...
Plan State 是 Harness Runtime 的关键证据来源。最终答复进入 `pre_final` gate 时，harness 会读取当前 plan snapshot；如果计划仍有非终态项，候选最终答复会被拒绝并生成 correction prompt。完整执行校验设计见 [Agent Harness Runtime 设计](./agent-harness-runtime.md)。
...
- `ExecutionStep` 仍记录工具、LLM 和 harness 校验时间线，不复用为计划载体
