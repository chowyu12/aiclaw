...
跨会话的个性化记忆系统，借鉴 Hermes Agent 设计：

- **MEMORY.md** — Agent 个人笔记：环境事实、项目惯例、工具使用经验（上限 2200 字符）
- **USER.md** — 用户画像：偏好、沟通风格、工作习惯（上限 1375 字符）
- 会话启动时加载冻结快照注入 system prompt，会话中修改下次生效
- 内置安全扫描，拦截注入攻击和不可见字符
...
- **会话笔记**: 每轮执行结束后自动将摘要追加到磁盘文件，下轮对话时注入 System Prompt，零 API 开销
- **跨会话搜索**: 内置 `session_search` 工具，基于 SQLite FTS5 全文索引搜索过去的对话历史。用户说「上次」「我们之前」时 Agent 自动调用
...
19 个内置工具，覆盖文件操作、命令执行、网页交互、桌面自动化、任务调度、记忆管理和 Agent 协作：
...
| `memory`           | 持久记忆管理：读写 MEMORY.md（Agent 笔记）和 USER.md（用户画像）|
...
- 持久记忆（Persistent Memory）：MEMORY.md + USER.md，跨会话保留 Agent 和用户知识
- 跨会话搜索（Session Search）：FTS5 全文索引快速检索历史对话
