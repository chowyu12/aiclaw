...
- **浏览器**：应用开一个独立的浏览器窗口，模型拿到页面上可交互元素的编号列表，按编号
  打开、点、填、选、滚、抽正文、截图（browser-use 那套 DOM indexing 的做法，用 Electron
  自带的 Chromium，不装 Playwright）。打开网址要确认，登录态跨会话保留，窗口可见随时
  能接手。与 browser-use 的逐项对照见 docs/design/browser.md。
