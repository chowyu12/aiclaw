...
采用运行时 **Plan State** 管理复杂任务进度：模型通过内部 `plan` 控制工具提出计划变更，执行器 harness 负责校验、持久化、流式下发和失败兜底。

- 复杂请求自动拆解为多个计划项（`pending` → `running` → `completed`，并支持 `blocked/failed/skipped`）
- 同一计划只允许一个 `running`，执行失败时 harness 自动将当前项标记为 `failed`
- 每轮 LLM 调用前只注入紧凑计划快照，不把计划正文混入最终回答
- 计划随 SSE 实时下发，聊天页和执行日志页均可回看最终快照
...
| L2 | **Plan State** (`plan` 控制工具) | LLM 提出计划变更，harness 校验持久化 | 每轮 LLM 调用前注入紧凑计划快照 | 当前任务进度规划 |
...
| `plan`             | 运行时计划状态：创建/修订/读取执行计划，流式展示并持久化快照    |
...
- 完整记忆体系详见上方 [「记忆架构」](#记忆架构) 章节，覆盖持久记忆、Plan State、会话归档、历史压缩、运行时压缩、Skill 结晶六层
- 在任意接入渠道支持 `/new` `/continue` `/help` 斜杠命令切换或续接会话
- 支持流式（SSE）和阻塞式两种响应模式
- 流式响应实时展示执行计划与执行步骤，done chunk 包含完整结果

### 执行日志

- 完整记录每次 Agent 调用的执行链路
- 展示每条 Agent 回复对应的 Plan State 快照
